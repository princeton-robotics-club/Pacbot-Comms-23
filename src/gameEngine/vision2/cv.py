...
class MovementProcessor(rm.ProtoModule):
    ...
    def tick(self):
        ...
        warped = warped[:,20:-20]
        # print(type(warped),warped.shape)
        ...
        # else:
        #mask = (np.all(bgr>yellow_low_bgr,axis = 2) & np.all(bgr < yellow_high_bgr,axis = 2)).astype(int)*255
        ...
        if len(contours) != 0:
            ...
            if True:
                ...
                cv2.rectangle(np.array(warped), (x, y), (x + w, y + h), (255,0,0), 4)
                print((b_x, b_y))
                if not skip:
